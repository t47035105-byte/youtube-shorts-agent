...
def resolve_voice_id() -> str:
    ...
    if voice_id:
        return voice_id
    voice_id_file = Path(os.environ.get("ELEVENLABS_VOICE_ID_FILE", str(DEFAULT_VOICE_ID_FILE)))
    if voice_id_file.exists():
        voice_id = voice_id_file.read_text(encoding="utf-8").strip()
        if voice_id:
            return voice_id
    raise RuntimeError("ELEVENLABS_VOICE_ID is required, or run the ElevenLabs voice setup workflow")
